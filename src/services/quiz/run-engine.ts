...
/**
 * Computes each section's absolute deadline (ms since epoch) from the run's
 * start time. Sections run back-to-back: the first section's budget starts at
 * `startedAt` and every later section inherits the previous one's expiry.
 *
 * A section with no budget (`timeLimitSeconds <= 0`) has no deadline.
 */
export function sectionDeadlines(
  ...
  for (const section of [...sections].sort((a, b) => a.position - b.position)) {
    if (section.timeLimitSeconds > 0) {
      cursor += section.timeLimitSeconds * 1000;
      deadlines.set(section.code, cursor);
    }
  }
  return deadlines;
}
...
/**
 * The effective deadline for an item, used to enforce per-section timing on the
 * server. Falls back to the overall run expiry when no section deadline exists
 * for the item's section (e.g. plain quizzes).
 */
function itemDeadline(run: RunRow, itemSectionCode: string | null): number | null {
  const section = run.sections.find((entry) => entry.code === itemSectionCode);
  if (section && section.timeLimitSeconds > 0) {
    const deadlines = sectionDeadlines(new Date(run.startedAt).getTime(), run.sections);
    return deadlines.get(section.code) ?? (run.expiresAt ? new Date(run.expiresAt).getTime() : null);
  }
  return run.expiresAt ? new Date(run.expiresAt).getTime() : null;
}
...
export async function answerRunItem(input: {
  ...
  // Time enforcement, overall first then per-section: a timed item whose section
  // budget (or the run's overall clock) has elapsed can no longer be answered.
  const deadline = itemDeadline(run, item.sectionCode);
  const overallExpired = run.expiresAt && new Date(run.expiresAt).getTime() <= Date.now();
  const sectionExpired = deadline !== null && deadline <= Date.now();
  if (overallExpired || sectionExpired) {
