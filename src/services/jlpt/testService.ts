...
export class TestService {
  ...
  static async submitSession(
    ...
    /* ---- Phase 12.1: XP for the exam, plus per-question XP ---- */
    const xpUserId = session.userId || "anonymous-user";
    await XpService.awardSafe({
      payload: {
        eventType: "quiz.test_completed",
        userId: xpUserId,
        sourceType: "jlpt_session",
        sourceId: sessionId,
        jlptLevel: session.jlptLevel,
        scorePercent: result.percentage,
        passed: result.passed,
        totalQuestions: result.evaluations.length,
      },
      dedupeKey: `jlpt-test:${sessionId}`,
    });
    ...
    for (const ev of result.evaluations) {
      await XpService.awardSafe({
        payload: {
          ...
          userId: xpUserId,
          sourceType: "jlpt_answer",
          sourceId: `${sessionId}:${ev.questionId}`,
          wasCorrect: ev.isCorrect,
          jlptLevel: session.jlptLevel,
          questionDifficulty: ev.question.difficulty,
        },
        dedupeKey: `jlpt-answer:${sessionId}:${ev.questionId}`,
      });
    }
    await XpService.awardDailyStreak(xpUserId);
