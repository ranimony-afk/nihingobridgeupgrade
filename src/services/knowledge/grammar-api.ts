...
export async function getGrammarStructures(slug: string) {
  const point = await getGrammarPointBySlug(slug);
  if (!point) return null;
  return {
    point: { id: point.id, slug: point.slug, title: point.title },
    structures: await listGrammarStructures(point.id),
  };
}

export async function getGrammarMistakes(
  slug: string,
  options: { severity?: string } = {},
) {
  const point = await getGrammarPointBySlug(slug);
  if (!point) return null;
  const all = await listGrammarMistakes(point.id);
  const mistakes = options.severity
    ? all.filter((mistake) => mistake.severity === options.severity)
    : all;
  return {
    point: { id: point.id, slug: point.slug, title: point.title },
    mistakes,
