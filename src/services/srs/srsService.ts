...
export class SrsService {
  ...
  static async gradeCard(input: {
    ...
    /* ---- Phase 12.1: emit an XP event (idempotent, non-fatal) ----
     * Keyed on the reviewId so a Phase 11.4 sync replay cannot double-pay,
     * and so Phase 11.2 undo can revoke exactly this award. */
    await XpService.awardSafe({
      payload: {
        ...
        wasCorrect: outcome.wasCorrect,
        intervalDays: outcome.intervalDays,
        isNewCard: stateBefore.totalReviews === 0,
        isLapse: input.rating === "again",
        cardType: cardRow.cardType,
        schedulerKey: scheduler.key,
      },
      dedupeKey: `review:${reviewId}`,
    });
    await XpService.awardDailyStreak(cardRow.userId);
