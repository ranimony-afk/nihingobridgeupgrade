...
/** Renders a sentence with the matched grammar pattern highlighted. */
function ExampleSentence({ example }: { example: GrammarExample }) {
  ...
  for (const range of ranges) {
    ...
    parts.push({ text: example.japanese.slice(range.startIndex, range.endIndex), highlight: true });
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-slate-600">{example.english}</p>
      {example.externalId ? (
