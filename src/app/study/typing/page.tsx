...
export default async function TypingStudyPage({
  searchParams,
}: {
  ...
  const cards = deckId
    ...
    : await db.select().from(customDeckCards).orderBy(asc(customDeckCards.position)).limit(20);
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <h1 className="mt-1 text-2xl font-bold" style={{ color: cfg.theme.primary }}>
              Typing &amp; Spelling Study Mode ⌨️
            </h1>
          </div>
        </div>

        <WriteClient cards={cards} />
      </div>
    </main>
  );
}
