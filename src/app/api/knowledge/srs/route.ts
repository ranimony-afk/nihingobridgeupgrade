...
export async function POST(request: NextRequest) {
  try {
    ...
    if (target === "kana") {
      ...
      const xp = result.created
        ? await XpService.awardSafe({
            ...
            dedupeKey: `knowledge:${result.cardId}`,
          })
        : null;
      return NextResponse.json({ success: true, ...result, xp });
    }
    ...
    if (target === "kana-row") {
      ...
      const xp = result.created > 0
        ? await XpService.awardSafe({
            payload: {
              eventType: "knowledge.card_added",
              userId: body.userId || "anonymous-user",
              sourceType: "kana_chart_row",
              sourceId: `${body.script}:${body.rowKey}`,
              knowledgeKind: "kana",
              cardsCreated: result.created,
            },
            dedupeKey: `knowledge-row:${body.script}:${body.rowKey}`,
          })
        : null;
      return NextResponse.json({ success: true, ...result, xp });
    }
