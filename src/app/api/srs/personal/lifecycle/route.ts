...
export async function POST(request: NextRequest) {
  try {
    ...
    // IMPORTANT: the session planner picks new cards by dueAt ASC (the "due"
    // order). Selecting the probe the same way guarantees the gate observes a
    // card the planner will actually serve, rather than an arbitrary one.
    let [probe] = await db
      .select()
      .from(cardsTable)
    ...
    // Fall back to the earliest-due review card when no new cards remain.
    if (!probe) {
      [probe] = await db
        .select()
        .from(cardsTable)
    ...
    // The public session shape exposes only queueSize; read the raw plan to
    // confirm the exact cursor position the card was served at.
