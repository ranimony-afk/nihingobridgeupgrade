...
/**
 * POST /api/v1/mobile/reviews { cardId, quality (0-5) }
 */
export async function POST(req: Request) {
  try {
    const body = (await req.json()) as { cardId: number; quality: number };
    if (!body.cardId || typeof body.quality !== "number") {
      return fail("cardId and quality (0-5) required", 400, "BAD_REQUEST");
    }

    const rows = await db.select().from(customDeckCards).where(eq(customDeckCards.id, body.cardId)).limit(1);
