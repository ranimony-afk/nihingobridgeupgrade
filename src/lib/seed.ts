...
async function doSeed(): Promise<void> {
  ...
  // Programmatic Self-Healing Migration (Phase 20 / Prompt 20 Hardening)
  try {
    const { migrate } = await import("drizzle-orm/node-postgres/migrator");
    console.log("⚙️  Self-Healing: Auto-migrating database on cold start...");
    await migrate(db, { migrationsFolder: "drizzle" });
    console.log("✅ Self-Healing: Migration complete!");
  } catch (migErr) {
    console.warn("⚠️ Self-Healing: Auto-migration skipped or failed:", (migErr as Error).message);
  }
