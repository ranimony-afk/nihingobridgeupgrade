...
export async function resolveCoupon(codeRaw: string | undefined) {
  ...
  if (code.startsWith("AFFILIATE-")) {
    // Phase 13: a code only discounts when it maps to an active partner.
    const { findAffiliate } = await import("./affiliate");
    const affiliate = await findAffiliate(code);
    if (!affiliate) return { ok: false as const, error: "Unknown or inactive affiliate code" };
    return {
      ok: true as const,
...
export async function createCheckout(input: {
  ...
  await db.insert(billingCheckouts).values({
    ...
    referralCode:
      coupon?.kind === "referral" || coupon?.kind === "affiliate" ? coupon.code : null,
    currency: plan.currency,
...
export async function fulfillCheckout(checkoutId: string, providerRef?: string) {
  ...
  // Phase 13 attribution: referral ledger + affiliate commission.
  const { accrueCommission, recordReferral } = await import("./affiliate");
  if (referrerId) {
    await recordReferral({
      referrerId,
      referredId: checkout.userId,
      checkoutId: checkout.id,
      rewardAmount: 1000,
      currency: checkout.currency,
    });
  }
  if (checkout.referralCode?.startsWith("AFFILIATE-")) {
    await accrueCommission({
      code: checkout.referralCode,
      checkoutId: checkout.id,
      invoiceId,
      currency: checkout.currency,
      netAmount: checkout.total,
    });
  }
...
export async function refundInvoice(invoiceId: string, reason: string) {
  ...
  // A refunded sale must not pay an affiliate.
  const { reverseCommission } = await import("./affiliate");
  await reverseCommission(invoiceId);

  await db
    .update(billingSubscriptions)
    .set({ status: "canceled" })
    .where(and(eq(billingSubscriptions.userId, invoice.userId), eq(billingSubscriptions.status, "active")));

  return { ok: true };
}

/** Cancels at period end — the learner keeps access they already paid for. */
export async function cancelSubscription(userId: string) {
  ...
  if (!sub) return { ok: false as const, error: "No active subscription", status: 404 };

  await db.update(billingSubscriptions).set({ status: "canceling" }).where(eq(billingSubscriptions.id, sub.id));
  return { ok: true as const, endsAt: sub.currentPeriodEnd };
}
...
export async function resumeSubscription(userId: string) {
  const [sub] = await db
    .select()
    .from(billingSubscriptions)
    .where(and(eq(billingSubscriptions.userId, userId), eq(billingSubscriptions.status, "canceling")))
    .orderBy(desc(billingSubscriptions.createdAt));
  if (!sub) return { ok: false as const, error: "Nothing to resume", status: 404 };
  if (sub.currentPeriodEnd.getTime() < Date.now()) {
    return { ok: false as const, error: "Subscription already lapsed", status: 400 };
  }

  await db.update(billingSubscriptions).set({ status: "active" }).where(eq(billingSubscriptions.id, sub.id));
  return { ok: true as const, endsAt: sub.currentPeriodEnd };
}

/**
 * Expires lapsed subscriptions and downgrades the plan.
 * Idempotent, so it is safe to call from a cron or on read.
 */
export async function expireLapsedSubscriptions() {
  ...
  for (const sub of rows) {
    ...
    if (sub.currentPeriodEnd.getTime() > now.getTime()) continue;
    await db.update(billingSubscriptions).set({ status: "expired" }).where(eq(billingSubscriptions.id, sub.id));
    await db.update(identityUsers).set({ plan: "free" }).where(eq(identityUsers.id, sub.userId));
    expired += 1;
  }
  return { expired };
}
...
/**
 * Records a webhook and reports whether it is a duplicate.
 * Providers retry aggressively, so replays must be a no-op.
 */
export async function recordWebhook(
  ...
  if (eventKey) {
    const existing = await db
      ...
      .where(and(eq(billingWebhookEvents.provider, provider), eq(billingWebhookEvents.eventKey, eventKey)));
