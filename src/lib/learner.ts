...
export async function toPublic(learner: typeof learners.$inferSelect): Promise<PublicLearner> {
  // Was two sequential round-trips (today's row, then a full week scan).
  // One aggregate does both, and runs on every authenticated page render.
  ...
  const [row] = (
    await db.execute<{
      ...
      SELECT
        COALESCE(SUM(xp) FILTER (WHERE date = ${todayKey()}), 0)::text AS xp,
        COALESCE(SUM(lessons_completed) FILTER (WHERE date = ${todayKey()}), 0)::text AS lessons,
        COALESCE(SUM(reviews_completed) FILTER (WHERE date = ${todayKey()}), 0)::text AS reviews,
        COALESCE(SUM(stories_completed) FILTER (WHERE date = ${todayKey()}), 0)::text AS stories,
        COALESCE(SUM(xp) FILTER (WHERE date >= ${weekStartKey()}), 0)::text AS weekly
      FROM daily_xp WHERE learner_id = ${learner.id}
    `)
  ).rows;
  const today = {
    xp: Number(row?.xp ?? 0),
    lessonsCompleted: Number(row?.lessons ?? 0),
    reviewsCompleted: Number(row?.reviews ?? 0),
    storiesCompleted: Number(row?.stories ?? 0),
  };
 ...
 * Leaderboard.
 *
 * Previously selected every learner and every weekly XP row into Node, then
 * joined and sorted in memory — unbounded as the user base grows. Now a single
 * grouped query does the aggregation, sort and limit in PostgreSQL.
 */
export async function getLeaderboard(limit = 50) {
  ...
  const rows = await db.execute<{
    ...
    SELECT l.id, l.name, l.avatar, l.xp, l.streak, l.is_bot,
      COALESCE(SUM(d.xp) FILTER (WHERE d.date >= ${weekStartKey()}), 0)::text AS weekly_xp
    FROM learners l
    LEFT JOIN daily_xp d ON d.learner_id = l.id
    GROUP BY l.id, l.name, l.avatar, l.xp, l.streak, l.is_bot
    -- Order by the numeric SUM, not the ::text alias. Sorting the text form
    -- would rank "9" above "100" lexicographically.
    ORDER BY COALESCE(SUM(d.xp) FILTER (WHERE d.date >= ${weekStartKey()}), 0) DESC, l.xp DESC
    LIMIT ${limit}
  `);
  return rows.rows.map((row) => ({
    ...
    name: row.name,
    avatar: row.avatar,
    avatarSrc: avatarLooks[row.avatar]?.src ?? avatarLooks.mochi.src,
    xp: Number(row.xp),
