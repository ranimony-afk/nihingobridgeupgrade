...
import { ensureCmsSeed } from "@/lib/cms/service";
import { enrichDictionary } from "@/lib/dict/enrich";
import { importGrammarCore } from "@/lib/grammar/engine";
...
export async function ensureKgSeed() {
  ...
  const explorer = await db.select().from(systemSettings).where(eq(systemSettings.key, "phase7_kanji"));
  if (explorer.length === 0) {
    await enrichKanjiExplorer();
  }
  const grammar = await db.select().from(systemSettings).where(eq(systemSettings.key, "phase8_grammar"));
  if (grammar.length === 0) {
    await importGrammarCore();
  }
  await ensureCmsSeed();
  const { ensureSearchSeed } = await import("@/lib/search/seed");
  await ensureSearchSeed();
