...
const environmentSchema = z
  .object({
    ...
    KNOWLEDGE_DATA_DIR: z.string().trim().min(1).default("./data/knowledge"),
    KNOWLEDGE_IMPORT_BATCH_SIZE: z.coerce.number().int().min(50).max(10_000).default(500),
    KNOWLEDGE_MAX_VALIDATION_ISSUES: z.coerce.number().int().min(100).max(1_000_000).default(10_000),
...
const parsed = environmentSchema.safeParse({
  ...
  KNOWLEDGE_IMPORT_BATCH_SIZE: process.env.KNOWLEDGE_IMPORT_BATCH_SIZE,
  KNOWLEDGE_MAX_VALIDATION_ISSUES: process.env.KNOWLEDGE_MAX_VALIDATION_ISSUES,
