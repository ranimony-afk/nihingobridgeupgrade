...
export const searchQuery = z.object({
  ...
  mode: z.enum(["auto", "exact", "full_text", "fuzzy"]).default("auto"),
  types: z
    .string()
    .optional()
    .transform((value, context) => {
      const allowed = ["dictionary", "kanji", "grammar", "sentence", "course", "lesson"] as const;
      const rawValues = (value ?? "dictionary,kanji,grammar,sentence,course,lesson")
        .split(",")
        .map((part) => part.trim())
        .filter(Boolean);
      const values = rawValues.map((item) => (item === "vocabulary" ? "dictionary" : item));
      const invalid = values.filter(
        (item) => !allowed.includes(item as (typeof allowed)[number]),
      );
      if (invalid.length > 0) {
        context.addIssue({
          code: "custom",
          message: `unknown search type(s): ${invalid.join(", ")}`,
        });
        return z.NEVER;
      }
      return Array.from(new Set(values)) as Array<(typeof allowed)[number]>;
    }),
  jlpt: jlpt.optional(),
  limit: z.coerce.number().int().min(1).max(100).default(24),
  offset: z.coerce.number().int().min(0).default(0),
  threshold: z.coerce.number().min(0.1).max(0.95).default(0.38),
});
...
export const searchSuggestQuery = z.object({
  ...
  types: z
    ...
    .transform((value) => {
      ...
      return Array.from(
        new Set(
          (value ?? "dictionary,kanji,grammar,sentence,course,lesson")
            .split(",")
            .map((part) => part.trim())
            .map((part) => (part === "vocabulary" ? "dictionary" : part))
            .filter((part): part is (typeof allowed)[number] =>
              allowed.includes(part as (typeof allowed)[number]),
            ),
        ),
      );
    }),
