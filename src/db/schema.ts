...
/** Japanese knowledge datasets, licenses, attribution, and source revision state. */
export const knowledgeDatasets = pgTable(
  "knowledge_datasets",
  {
    id: text("id")
      .primaryKey()
    ...
    latestChecksum: varchar("latest_checksum", { length: 128 }),
    metadata: jsonb("metadata").$type<Record<string, unknown>>().notNull().default({}),
    createdAt: timestamp("created_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
  ...
  (table) => [uniqueIndex("knowledge_datasets_key_unique").on(table.key)],
);

/** A source-file import is versioned, checksummed, resumable, and auditable. */
export const knowledgeImportRuns = pgTable(
  ...
  (table) => [
    uniqueIndex("knowledge_import_runs_dataset_revision_unique").on(table.datasetId, table.sourceVersion, table.sourceChecksum),
    index("knowledge_import_runs_dataset_status_idx").on(table.datasetId, table.status),
  ],
);

/** Validation and parser diagnostics retained per source record without aborting a full import. */
export const knowledgeValidationIssues = pgTable(
  "knowledge_validation_issues",
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    importRunId: text("import_run_id")
      .notNull()
      .references(() => knowledgeImportRuns.id, { onDelete: "cascade" }),
  ...
  (table) => [
    index("knowledge_validation_issues_run_severity_idx").on(table.importRunId, table.severity),
    index("knowledge_validation_issues_code_idx").on(table.code),
  ],
);

/** Curated and dataset-derived semantic categories applied across all entity types. */
export const knowledgeSemanticTags = pgTable(
  "knowledge_semantic_tags",
  {
    id: text("id")
      .primaryKey()
      .$defaultFn(() => crypto.randomUUID()),
    ...
    category: varchar("category", { length: 64 }).notNull().default("semantic"),
    description: text("description"),
    createdAt: timestamp("created_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
  },
  (table) => [uniqueIndex("knowledge_semantic_tags_slug_unique").on(table.slug)],
);

/** JMdict-level lexical entries; spellings/readings/senses are normalized below. */
export const knowledgeLexemes = pgTable(
  "knowledge_lexemes",
  {
    id: text("id")
      .primaryKey()
    ...
    primarySpelling: text("primary_spelling"),
    primaryReading: text("primary_reading"),
    primaryGloss: text("primary_gloss"),
    common: boolean("common").notNull().default(false),
    jlptLevel: varchar("jlpt_level", { length: 8 }),
  ...
  (table) => [
    uniqueIndex("knowledge_lexemes_dataset_external_unique").on(table.datasetId, table.externalId),
    index("knowledge_lexemes_spelling_idx").on(table.primarySpelling),
    index("knowledge_lexemes_reading_idx").on(table.primaryReading),
    index("knowledge_lexemes_jlpt_frequency_idx").on(table.jlptLevel, table.frequencyRank),
  ],
);

export const knowledgeLexemeSpellings = pgTable(
  "knowledge_lexeme_spellings",
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    lexemeId: text("lexeme_id")
      .notNull()
      .references(() => knowledgeLexemes.id, { onDelete: "cascade" }),
    spelling: text("spelling").notNull(),
    priority: integer("priority").notNull().default(0),
    isPrimary: boolean("is_primary").notNull().default(false),
    information: jsonb("information").$type<string[]>().notNull().default([]),
  },
  (table) => [
    uniqueIndex("knowledge_lexeme_spellings_unique").on(table.lexemeId, table.spelling),
    index("knowledge_lexeme_spellings_spelling_idx").on(table.spelling),
  ],
);

export const knowledgeLexemeReadings = pgTable(
  "knowledge_lexeme_readings",
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    lexemeId: text("lexeme_id")
      .notNull()
    ...
    isPrimary: boolean("is_primary").notNull().default(false),
    pitchAccents: jsonb("pitch_accents").$type<Array<{ pattern: number; morae: string[]; source?: string }>>().notNull().default([]),
    furigana: jsonb("furigana").$type<Array<{ ruby: string; rt: string }>>().notNull().default([]),
    information: jsonb("information").$type<string[]>().notNull().default([]),
  },
  (table) => [
    uniqueIndex("knowledge_lexeme_readings_unique").on(table.lexemeId, table.reading),
    index("knowledge_lexeme_readings_reading_idx").on(table.reading),
  ],
);

export const knowledgeLexemeSenses = pgTable(
  "knowledge_lexeme_senses",
  {
    id: text("id")
      .primaryKey()
      .$defaultFn(() => crypto.randomUUID()),
    ...
    partOfSpeech: jsonb("part_of_speech").$type<string[]>().notNull().default([]),
    fields: jsonb("fields").$type<string[]>().notNull().default([]),
    dialects: jsonb("dialects").$type<string[]>().notNull().default([]),
    misc: jsonb("misc").$type<string[]>().notNull().default([]),
    appliesToSpellings: jsonb("applies_to_spellings").$type<string[]>().notNull().default([]),
    appliesToReadings: jsonb("applies_to_readings").$type<string[]>().notNull().default([]),
    searchText: text("search_text").notNull().default(""),
  },
  (table) => [
    uniqueIndex("knowledge_lexeme_senses_position_unique").on(table.lexemeId, table.position),
    index("knowledge_lexeme_senses_lexeme_idx").on(table.lexemeId),
  ],
);

export const knowledgeLexemeGlosses = pgTable(
  "knowledge_lexeme_glosses",
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    senseId: text("sense_id")
      .notNull()
      .references(() => knowledgeLexemeSenses.id, { onDelete: "cascade" }),
    language: varchar("language", { length: 16 }).notNull().default("eng"),
  ...
  (table) => [
    index("knowledge_lexeme_glosses_language_gloss_idx").on(table.language, table.gloss),
    index("knowledge_lexeme_glosses_sense_idx").on(table.senseId),
  ],
);

/** JMnedict proper names stay separate from general lexemes to preserve source semantics. */
export const knowledgeNames = pgTable(
  "knowledge_names",
  {
    id: text("id")
      .primaryKey()
    ...
    kanji: text("kanji"),
    reading: text("reading").notNull(),
    nameTypes: jsonb("name_types").$type<string[]>().notNull().default([]),
  ...
  (table) => [
    uniqueIndex("knowledge_names_dataset_external_unique").on(table.datasetId, table.externalId),
    index("knowledge_names_kanji_idx").on(table.kanji),
    index("knowledge_names_reading_idx").on(table.reading),
  ],
);

/** KANJIDIC2/Unihan kanji character records. */
export const knowledgeKanji = pgTable(
  "knowledge_kanji",
  {
    id: text("id")
      .primaryKey()
    ...
    unicodeCodepoint: varchar("unicode_codepoint", { length: 16 }).notNull(),
    radical: varchar("radical", { length: 16 }),
    grade: integer("grade"),
    strokeCount: integer("stroke_count"),
    frequencyRank: integer("frequency_rank"),
    jlptLevel: varchar("jlpt_level", { length: 8 }),
    joyo: boolean("joyo").notNull().default(false),
    jinmeiyo: boolean("jinmeiyo").notNull().default(false),
    searchText: text("search_text").notNull().default(""),
    sourceHash: varchar("source_hash", { length: 128 }).notNull(),
    updatedAt: timestamp("updated_at", { withTimezone: true })
      .notNull()
  ...
  (table) => [
    uniqueIndex("knowledge_kanji_literal_unique").on(table.literal),
    index("knowledge_kanji_jlpt_frequency_idx").on(table.jlptLevel, table.frequencyRank),
    index("knowledge_kanji_radical_idx").on(table.radical),
  ],
);

export const knowledgeKanjiReadings = pgTable(
  "knowledge_kanji_readings",
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    kanjiId: text("kanji_id")
      .notNull()
      .references(() => knowledgeKanji.id, { onDelete: "cascade" }),
    reading: text("reading").notNull(),
    kind: varchar("kind", { length: 32 }).notNull(),
    status: varchar("status", { length: 32 }),
  },
  (table) => [
    uniqueIndex("knowledge_kanji_readings_unique").on(table.kanjiId, table.reading, table.kind),
    index("knowledge_kanji_readings_reading_idx").on(table.reading),
  ],
);

export const knowledgeKanjiMeanings = pgTable(
  "knowledge_kanji_meanings",
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    kanjiId: text("kanji_id")
      .notNull()
  ...
  (table) => [
    uniqueIndex("knowledge_kanji_meanings_unique").on(table.kanjiId, table.language, table.meaning),
    index("knowledge_kanji_meanings_meaning_idx").on(table.meaning),
  ],
);

export const knowledgeKanjiComponents = pgTable(
  "knowledge_kanji_components",
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    kanjiId: text("kanji_id")
      .notNull()
      .references(() => knowledgeKanji.id, { onDelete: "cascade" }),
    componentLiteral: varchar("component_literal", { length: 8 }).notNull(),
    componentType: varchar("component_type", { length: 32 }).notNull().default("radical"),
    position: integer("position").notNull().default(0),
  },
  (table) => [
    uniqueIndex("knowledge_kanji_components_unique").on(table.kanjiId, table.componentLiteral, table.componentType),
    index("knowledge_kanji_components_literal_idx").on(table.componentLiteral),
  ],
);

/** KanjiVG path data remains source-attributed and can be rendered in any client. */
export const knowledgeKanjiStrokes = pgTable(
  "knowledge_kanji_strokes",
  {
    id: text("id")
      .primaryKey()
      .$defaultFn(() => crypto.randomUUID()),
  ...
  (table) => [
    uniqueIndex("knowledge_kanji_strokes_unique").on(table.kanjiId, table.datasetId, table.strokeNumber),
    index("knowledge_kanji_strokes_kanji_idx").on(table.kanjiId),
  ],
);

/** Tatoeba and controlled sentence corpora; translations and UniDic tokens are normalized. */
export const knowledgeSentences = pgTable(
  "knowledge_sentences",
  {
    id: text("id")
      .primaryKey()
  ...
  (table) => [
    uniqueIndex("knowledge_sentences_dataset_external_unique").on(table.datasetId, table.externalId),
    index("knowledge_sentences_language_jlpt_idx").on(table.language, table.jlptLevel),
  ],
);

export const knowledgeSentenceTranslations = pgTable(
  "knowledge_sentence_translations",
  {
    id: text("id")
      .primaryKey()
      .$defaultFn(() => crypto.randomUUID()),
    sentenceId: text("sentence_id")
      .notNull()
      .references(() => knowledgeSentences.id, { onDelete: "cascade" }),
    datasetId: text("dataset_id")
      .notNull()
      .references(() => knowledgeDatasets.id, { onDelete: "restrict" }),
  ...
  (table) => [
    uniqueIndex("knowledge_sentence_translations_unique").on(table.sentenceId, table.language, table.text),
    index("knowledge_sentence_translations_language_idx").on(table.language),
  ],
);

export const knowledgeSentenceTokens = pgTable(
  "knowledge_sentence_tokens",
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    sentenceId: text("sentence_id")
      .notNull()
    ...
    partOfSpeech: varchar("part_of_speech", { length: 128 }),
    inflectionType: varchar("inflection_type", { length: 128 }),
    inflectionForm: varchar("inflection_form", { length: 128 }),
    startOffset: integer("start_offset"),
    endOffset: integer("end_offset"),
    features: jsonb("features").$type<Record<string, string>>().notNull().default({}),
  },
  (table) => [
    uniqueIndex("knowledge_sentence_tokens_position_unique").on(table.sentenceId, table.position),
    index("knowledge_sentence_tokens_lemma_idx").on(table.lemma),
  ],
);

/** Grammar, idioms, and collocations are first-class graph entities with examples. */
export const knowledgeGrammarPoints = pgTable(
  "knowledge_grammar_points",
  {
    id: text("id")
      .primaryKey()
      .$defaultFn(() => crypto.randomUUID()),
  ...
  (table) => [
    uniqueIndex("knowledge_grammar_points_dataset_external_unique").on(table.datasetId, table.externalId),
    index("knowledge_grammar_points_jlpt_idx").on(table.jlptLevel),
    index("knowledge_grammar_points_pattern_idx").on(table.pattern),
  ],
);

export const knowledgeGrammarExamples = pgTable(
  "knowledge_grammar_examples",
  {
    id: text("id")
      .primaryKey()
      .$defaultFn(() => crypto.randomUUID()),
    grammarPointId: text("grammar_point_id")
      .notNull()
      .references(() => knowledgeGrammarPoints.id, { onDelete: "cascade" }),
    sentenceId: text("sentence_id").references(() => knowledgeSentences.id, { onDelete: "set null" }),
    japanese: text("japanese").notNull(),
    english: text("english"),
    explanation: text("explanation"),
    position: integer("position").notNull().default(0),
  },
  (table) => [index("knowledge_grammar_examples_point_idx").on(table.grammarPointId)],
);

export const knowledgeIdioms = pgTable(
  "knowledge_idioms",
  {
    id: text("id")
      .primaryKey()
    ...
    reading: text("reading"),
    meaning: text("meaning").notNull(),
    register: varchar("register", { length: 64 }),
    jlptLevel: varchar("jlpt_level", { length: 8 }),
    searchText: text("search_text").notNull().default(""),
    sourceHash: varchar("source_hash", { length: 128 }).notNull(),
    updatedAt: timestamp("updated_at", { withTimezone: true })
  ...
  (table) => [
    uniqueIndex("knowledge_idioms_dataset_external_unique").on(table.datasetId, table.externalId),
    index("knowledge_idioms_expression_idx").on(table.expression),
  ],
);

export const knowledgeCollocations = pgTable(
  "knowledge_collocations",
  {
    id: text("id")
      .primaryKey()
      .$defaultFn(() => crypto.randomUUID()),
    ...
    collocate: text("collocate").notNull(),
    relation: varchar("relation", { length: 64 }).notNull().default("cooccurrence"),
  ...
  (table) => [
    uniqueIndex("knowledge_collocations_dataset_external_unique").on(table.datasetId, table.externalId),
    index("knowledge_collocations_headword_idx").on(table.headword),
    index("knowledge_collocations_collocate_idx").on(table.collocate),
  ],
);

/** Open or licensed audio references are source-attributed and never embedded as binary database blobs. */
export const knowledgeAudioAssets = pgTable(
  "knowledge_audio_assets",
  {
    id: text("id")
      .primaryKey()
      .$defaultFn(() => crypto.randomUUID()),
  ...
  (table) => [
    uniqueIndex("knowledge_audio_assets_url_unique").on(table.url),
    index("knowledge_audio_assets_entity_idx").on(table.entityType, table.entityId),
  ],
);

/** Generic semantic links maintain cross-domain tagging without widening every entity table. */
export const knowledgeEntityTags = pgTable(
  "knowledge_entity_tags",
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    entityType: varchar("entity_type", { length: 64 }).notNull(),
    entityId: text("entity_id").notNull(),
    semanticTagId: text("semantic_tag_id")
      .notNull()
      .references(() => knowledgeSemanticTags.id, { onDelete: "cascade" }),
    confidenceBps: integer("confidence_bps").notNull().default(10000),
    provenance: varchar("provenance", { length: 32 }).notNull().default("source"),
  },
  (table) => [
    uniqueIndex("knowledge_entity_tags_unique").on(table.entityType, table.entityId, table.semanticTagId),
    index("knowledge_entity_tags_tag_idx").on(table.semanticTagId),
  ],
);

/** AI enrichments are versioned against entity content and never overwrite canonical source facts. */
export const knowledgeAiMetadata = pgTable(
  "knowledge_ai_metadata",
  {
    id: text("id")
      .primaryKey()
      .$defaultFn(() => crypto.randomUUID()),
  ...
  (table) => [
    uniqueIndex("knowledge_ai_metadata_version_unique").on(table.entityType, table.entityId, table.model, table.promptVersion, table.contentHash),
    index("knowledge_ai_metadata_status_idx").on(table.status),
  ],
);

/** User-specific spaced-repetition state for any knowledge entity. */
export const knowledgeSrsCards = pgTable(
  "knowledge_srs_cards",
  {
    id: text("id")
      .primaryKey()
    ...
    easeFactorBps: integer("ease_factor_bps").notNull().default(250),
  ...
  (table) => [
    uniqueIndex("knowledge_srs_cards_user_entity_unique").on(table.userId, table.entityType, table.entityId),
    index("knowledge_srs_cards_user_due_idx").on(table.userId, table.dueAt),
  ],
);

export const knowledgeSrsReviews = pgTable(
  "knowledge_srs_reviews",
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    cardId: text("card_id")
      .notNull()
      .references(() => knowledgeSrsCards.id, { onDelete: "cascade" }),
    rating: integer("rating").notNull(),
    previousIntervalDays: integer("previous_interval_days").notNull(),
    nextIntervalDays: integer("next_interval_days").notNull(),
    previousEaseFactorBps: integer("previous_ease_factor_bps").notNull(),
    nextEaseFactorBps: integer("next_ease_factor_bps").notNull(),
    reviewedAt: timestamp("reviewed_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
  },
  (table) => [index("knowledge_srs_reviews_card_reviewed_idx").on(table.cardId, table.reviewedAt)],
);
