...
/* -------------------------------------------------------------------------- */
/* JLPT test blueprints (phase 10.3)                                          */
/* -------------------------------------------------------------------------- */

/**
 * A published, timed JLPT mock-test blueprint.
 *
 * The blueprint stores *structure*, never content: each section declares which
 * skills/kinds it draws from and how many questions it needs. Questions are
 * sampled at attempt time from the canonical bank, so a blueprint is always in
 * sync with the bank and never duplicates question data.
 *
 * Section "listening" (聴解) is intentionally out of scope until the audio
 * domain exists; blueprints record `metadata.listeningPlanned` for clarity.
 */
export const jlptTests = pgTable(
  "jlpt_tests",
  {
    id: serial("id").primaryKey(),
    slug: text("slug").notNull(),
    /** Stable regenerable identity, e.g. `jlpt-n5-mock-1` */
    sourceKey: text("source_key").notNull(),
    /** 5 = N5 .. 1 = N1 */
    ...
    description: text("description"),
    instructions: text("instructions"),
    /** Total time limit across all sections. */
    timeLimitSeconds: integer("time_limit_seconds").notNull(),
    questionCount: integer("question_count").notNull(),
    totalPoints: integer("total_points").notNull(),
    /** Simplified overall pass mark (percent). */
    passingPercent: integer("passing_percent").notNull().default(50),
    /** Simplified per-section minimum (percent), mirroring JLPT section gates. */
    sectionMinimumPercent: integer("section_minimum_percent").notNull().default(35),
    /** False when the bank cannot yet satisfy every section. */
  ...
  (table) => [
    uniqueIndex("jlpt_tests_slug_unique").on(table.slug),
    uniqueIndex("jlpt_tests_source_key_unique").on(table.sourceKey),
    index("jlpt_tests_level_idx").on(table.level, table.published),
  ],
);

/** One scored part of a blueprint (文字・語彙, 文法・読解, ...). */
export const jlptTestSections = pgTable(
  "jlpt_test_sections",
  {
    id: serial("id").primaryKey(),
    testId: integer("test_id")
      .notNull()
      .references(() => jlptTests.id, { onDelete: "cascade" }),
    /** Stable section code, e.g. `moji-goi` */
    code: text("code").notNull(),
    title: text("title").notNull(),
    titleJa: text("title_ja"),
    instructions: text("instructions"),
    ...
    timeLimitSeconds: integer("time_limit_seconds").notNull(),
    questionCount: integer("question_count").notNull(),
    /** Bank skills this section samples from. */
    skills: jsonb("skills").$type<string[]>(),
    /** Bank kinds this section samples from (empty = any). */
    kinds: jsonb("kinds").$type<string[]>(),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
  },
  (table) => [
    uniqueIndex("jlpt_test_sections_code_unique").on(table.testId, table.code),
    index("jlpt_test_sections_test_idx").on(table.testId, table.position),
  ],
);

/* -------------------------------------------------------------------------- */
/* Quiz / JLPT assessment runs (phase 10.2)                                   */
/* -------------------------------------------------------------------------- */

/**
 * THE canonical run/attempt model for every graded assessment.
 *
 * A quiz and a JLPT attempt are the *same* thing — a fixed, ordered sample of
 * bank questions owned by one learner — so both live here and differ only by
 * `kind`. This is what keeps phase 10 from growing a second learning engine:
 * sampling, grading and scoring all go through the question engine, and only
 * the framing (sections, time limits, pass marks) differs.
 */
export const quizRuns = pgTable(
  "quiz_runs",
  {
    id: serial("id").primaryKey(),
    /** Opaque external id used by URLs and the API. */
    publicId: text("public_id").notNull(),
    userId: integer("user_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    ...
    status: text("status").notNull().default("in_progress"),
    title: text("title").notNull(),
    /** Set for JLPT attempts. */
    jlptTestId: integer("jlpt_test_id").references(() => jlptTests.id, {
      onDelete: "set null",
    }),
    jlptSlug: text("jlpt_slug"),
    /** Sampling seed — makes every run reproducible and auditable. */
    seed: integer("seed").notNull().default(0),
    jlptLevel: integer("jlpt_level"),
    skills: jsonb("skills").$type<string[]>(),
    kinds: jsonb("kinds").$type<string[]>(),
    /** Snapshot of section framing (code/title/limits) at start time. */
  ...
  (table) => [
    uniqueIndex("quiz_runs_public_id_key").on(table.publicId),
    index("quiz_runs_user_kind_idx").on(table.userId, table.kind, table.startedAt),
    index("quiz_runs_status_idx").on(table.status),
    index("quiz_runs_test_idx").on(table.jlptTestId),
  ],
);

/**
 * One row per question inside a run, in fixed order.
 *
 * `correct`/`awarded_points`/`feedback`/`explanation` are only filled once the
 * learner has answered: an unanswered item carries no answer-key information,
 * and a public read of a run never exposes the key for items still open.
 */
export const quizRunItems = pgTable(
  "quiz_run_items",
  {
    id: serial("id").primaryKey(),
    ...
    /** Section code for JLPT attempts, null for plain quizzes. */
    ...
    points: integer("points").notNull().default(1),
    awardedPoints: integer("awarded_points").notNull().default(0),
    /** Normalised accepted answer, revealed only after answering. */
  ...
  (table) => [
    uniqueIndex("quiz_run_items_position_unique").on(table.runId, table.position),
    index("quiz_run_items_run_idx").on(table.runId, table.questionId),
    index("quiz_run_items_question_idx").on(table.questionId),
  ],
);
