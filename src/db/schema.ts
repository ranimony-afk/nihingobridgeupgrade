...
/** Ordered teaching blocks inside one lesson. */
export const lessonSections = pgTable(
  "lesson_sections",
  {
    ...
    lessonId: integer("lesson_id")
      ...
      .references(() => lessons.id, { onDelete: "cascade" }),
    position: integer("position").notNull().default(0),
    /** explain | example | practice | note */
    kind: text("kind").notNull().default("explain"),
    heading: text("heading").notNull(),
    headingJa: text("heading_ja"),
  ...
  (table) => [
    uniqueIndex("lesson_sections_unique").on(table.lessonId, table.position),
    index("lesson_sections_lesson_idx").on(table.lessonId),
  ],
);

/**
 * Normalised learning objectives. `lessons.objectives` (JSONB) is retained for
 * compatibility and backfilled; this table is the canonical source.
 */
export const lessonObjectives = pgTable(
  ...
  (table) => [
    uniqueIndex("lesson_objectives_unique").on(table.lessonId, table.position),
    index("lesson_objectives_lesson_idx").on(table.lessonId),
  ],
);

/** Lesson -> canonical dictionary entry edge ("vocabulary taught here"). */
export const lessonVocabulary = pgTable(
  "lesson_vocabulary",
  {
    ...
    vocabularyId: integer("vocabulary_id")
      .notNull()
      .references(() => vocabulary.id, { onDelete: "cascade" }),
    /** kanji | grammar | manual */
    via: text("via").notNull().default("kanji"),
    position: integer("position").notNull().default(0),
  },
  (table) => [
...
/** Lesson -> canonical sentence edge ("example sentences used here"). */
export const lessonSentences = pgTable(
  "lesson_sentences",
  {
    id: serial("id").primaryKey(),
    lessonId: integer("lesson_id")
    ...
    /** grammar-example | manual */
    via: text("via").notNull().default("grammar-example"),
    position: integer("position").notNull().default(0),
  },
  (table) => [
    uniqueIndex("lesson_sentences_unique").on(table.lessonId, table.sentenceId),
    index("lesson_sentences_sentence_idx").on(table.sentenceId),
  ],
);
