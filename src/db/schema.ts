...
/** PostgreSQL tsvector column used by the Phase 12 search index. */
...
export const billingWebhookEvents = pgTable(
  "billing_webhook_events",
  {
    ...
    /** Provider event id. Unique so retried webhooks are processed once. */
    eventKey: text("event_key"),
    payload: jsonb("payload").$type<Record<string, unknown>>().notNull(),
    processed: boolean("processed").notNull().default(false),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  },
  (table) => [uniqueIndex("billing_webhook_event_key").on(table.provider, table.eventKey)],
);

/** Affiliate partners. A code only discounts if it maps to an active row here. */
export const billingAffiliates = pgTable("billing_affiliates", {
  id: text("id").primaryKey(),
  code: text("code").notNull().unique(),
  name: text("name").notNull(),
  email: text("email").notNull(),
  ...
  /** Percent of net revenue paid to the partner. */
  commissionPercent: integer("commission_percent").notNull().default(20),
  status: text("status").notNull().default("active"),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
});

/** One row per attributed sale. Immutable ledger. */
export const billingCommissions = pgTable("billing_commissions", {
  id: text("id").primaryKey(),
  affiliateId: text("affiliate_id")
    .notNull()
    .references(() => billingAffiliates.id, { onDelete: "cascade" }),
...
export const billingPayouts = pgTable("billing_payouts", {
  id: text("id").primaryKey(),
  affiliateId: text("affiliate_id")
    .notNull()
    .references(() => billingAffiliates.id, { onDelete: "cascade" }),
...
/** Referral attribution: who invited whom, and whether it converted. */
export const billingReferrals = pgTable(
  "billing_referrals",
  {
    ...
    status: text("status").notNull().default("converted"),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  },
  (table) => [uniqueIndex("billing_referral_pair").on(table.referrerId, table.referredId)],
);
...
export const kgRadicals = pgTable("kg_radicals", {
  id: text("id").primaryKey(),
  character: text("character").notNull().unique(),
...
export const kgKanjiRadicals = pgTable(
  "kg_kanji_radicals",
  {
    kanjiId: text("kanji_id")
      .notNull()
      .references(() => kgKanji.id, { onDelete: "cascade" }),
    radicalId: text("radical_id")
      .notNull()
      .references(() => kgRadicals.id, { onDelete: "cascade" }),
  },
  (table) => [uniqueIndex("kg_kanji_radicals_pk").on(table.kanjiId, table.radicalId)],
);

export const kgKanjiMeta = pgTable("kg_kanji_meta", {
  kanjiId: text("kanji_id")
    .primaryKey()
    .references(() => kgKanji.id, { onDelete: "cascade" }),
  ...
  rtkIndex: integer("rtk_index"),
  rtkKeyword: text("rtk_keyword"),
  wanikani: integer("wanikani"),
  nanori: text("nanori"),
});

export const kgKanjiEdges = pgTable(
  "kg_kanji_edges",
  {
    id: text("id").primaryKey(),
    fromId: text("from_id")
  ...
  (table) => [uniqueIndex("kg_kanji_edges_unique").on(table.fromId, table.toId, table.kind)],
);

export const kgGrammarMeta = pgTable("kg_grammar_meta", {
  grammarId: text("grammar_id")
    .primaryKey()
    .references(() => kgGrammar.id, { onDelete: "cascade" }),
  ...
  timeline: jsonb("timeline").$type<{ step: string; note: string }[]>().notNull(),
});

export const kgGrammarEdges = pgTable(
  "kg_grammar_edges",
  {
    id: text("id").primaryKey(),
    fromId: text("from_id").notNull(),
  ...
  (table) => [uniqueIndex("kg_grammar_edges_unique").on(table.fromId, table.toId, table.kind)],
);

export const kgGrammarExamples = pgTable("kg_grammar_examples", {
  id: text("id").primaryKey(),
  grammarId: text("grammar_id")
    .notNull()
    .references(() => kgGrammar.id, { onDelete: "cascade" }),
...
export const kgGrammarBuilder = pgTable("kg_grammar_builder", {
  id: text("id").primaryKey(),
  grammarId: text("grammar_id")
    .notNull()
    .references(() => kgGrammar.id, { onDelete: "cascade" }),
...
export const cmsCourses = pgTable("cms_courses", {
  ...
  status: text("status").notNull().default("draft"),
  modules: jsonb("modules").$type<{ title: string; lessons: string[] }[]>().notNull(),
});
...
export const cmsSeo = pgTable("cms_seo", {
...
/** Unified Elastic-style index across every content type. */
export const searchIndex = pgTable(
  ...
  (table) => [
    index("search_index_kind_idx").on(table.kind),
    index("search_index_jlpt_idx").on(table.jlpt),
    uniqueIndex("search_index_kind_ref").on(table.kind, table.refId),
  ],
);
...
export const searchSynonyms = pgTable(
  ...
  (table) => [uniqueIndex("search_synonyms_pair").on(table.term, table.expandsTo)],
);

/** Distinct suggestable terms (titles + gloss words) used for "did you mean". */
