...
async function httpChecks() {
  ...
  check("http: catalogue", catalog.status === 200, `${catalog.body?.total ?? 0} points`);
  check("http: catalogue non-empty", (catalog.body?.results ?? []).length > 20);

  const filtered = await json("/api/grammar?jlpt=4");
  check(
    "http: filter by JLPT level",
    filtered.status === 200 && (filtered.body?.results ?? []).every((point) => point.jlptLevel === 4),
    `${filtered.body?.total ?? 0} N4 points`,
  );

  const searched = await json(`/api/grammar?q=${encodeURIComponent("conditional")}`);
  check(
    "http: search grammar",
    searched.status === 200 && (searched.body?.results ?? []).length > 0,
    `${searched.body?.total ?? 0} results`,
  );

  const detail = await json(`/api/grammar/${SAMPLE_SLUG}`);
  check("http: grammar detail", detail.status === 200);
  check("http: detail has patterns", (detail.body?.patternDetails ?? []).length > 0);
  check("http: detail has examples", (detail.body?.examples ?? []).length > 0);
  check(
    "http: examples carry match offsets",
    (detail.body?.examples ?? []).every((example) => (example.matches ?? []).length > 0),
  );
  check("http: detail has related points", (detail.body?.related ?? []).length > 0);
  check("http: detail has kanji links", (detail.body?.kanji ?? []).length > 0);
  check("http: detail has provenance", (detail.body?.provenance ?? []).length > 0);
