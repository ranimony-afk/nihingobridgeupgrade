...
test("referral and affiliate codes resolve", async () => {
  assert.equal(await seedReady(), true);
  // Phase 13: an affiliate code must map to a real, active partner.
  // "AFFILIATE-PARTNER" was never a registered code — it only worked because
  // Phase 4 discounted any string with that prefix.
  const bogus = await resolveCoupon("AFFILIATE-PARTNER");
  assert.equal(bogus?.ok, false);

  const registered = await resolveCoupon("AFFILIATE-SAKURA");
  assert.equal(registered && "percentOff" in registered && registered.percentOff, 15);
});
