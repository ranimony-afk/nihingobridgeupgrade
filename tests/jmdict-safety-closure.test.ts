...
describe("14.3D short-batch checkpoint", () => {
  const dictionaryDdl = `CREATE TABLE dictionary_entries (
    id text PRIMARY KEY, headword text NOT NULL, reading text NOT NULL, romaji text NOT NULL,
    jlpt_level text NOT NULL, is_common boolean NOT NULL, frequency_rank integer,
    parts_of_speech jsonb NOT NULL, senses jsonb NOT NULL, kanji_characters jsonb NOT NULL,
    tags jsonb NOT NULL, source_ref text NOT NULL)`;

  it("persists version 2 after a successful batch smaller than the normal batch size", async () => {
    const client = new PGlite();
    const database = drizzle(client);
    state.database = database;
    try {
      await client.exec(dictionaryDdl);
      const adapter = new DrizzleDictionaryPersistenceAdapter();
      const source = capture(xml(9000101, "short batch"));
      const batch = await accepted(source);
      expect(batch.candidates.length).toBe(1);
      expect(batch.candidates.length).toBeLessThan(1000);
      const path = join(directory, "short-checkpoint.json");
      const committed = await commitSourceBatch(source, batch, adapter, false, {
        path,
        targetIdentity: target,
        progress: { ...progress, recordsProcessed: 1 },
      });
      expect(committed.inserted).toBe(1);
      const rows = await database.select().from(dictionaryEntries);
      expect(rows.map((row) => row.id)).toEqual(["de-jmdict-9000101"]);
      const saved = JSON.parse(fs.readFileSync(path, "utf8")) as IngestionCheckpoint;
      expect(saved.checkpointVersion).toBe(2);
      expect(saved.origin).toBe("ingestion");
      expect(saved.lastProcessedEntSeq).toBe("9000101");
      expect(saved.sourceId).toBe(source.sourceId);
      expect(saved.sourceHash).toBe(source.xmlSha256);
      expect(saved.releaseVersion).toBe(source.releaseVersion);
      expect(saved.transformationVersion).toBe(source.transformationVersion);
      expect(saved.schemaContract).toBe(source.schemaContract);
      expect(saved.deterministicIdStrategy).toBe(source.deterministicIdStrategy);
      expect(saved.targetIdentity).toEqual(target);
      expect(loadResumeCheckpoint(path, source, target).lastProcessedEntSeq).toBe("9000101");
    } finally {
      await client.close();
    }
  });

  it("does not leave partial rows or advance the checkpoint when a short batch fails", async () => {
    const client = new PGlite();
    const database = drizzle(client);
    state.database = database;
    try {
      await client.exec(dictionaryDdl);
      const adapter = new DrizzleDictionaryPersistenceAdapter();
      const prior = capture(xml(9000101, "prior"), "prior.xml");
      await adapter.upsertBatch((await accepted(prior)).candidates);
      const path = join(directory, "short-checkpoint.json");
      fs.writeFileSync(path, JSON.stringify(checkpoint({ lastProcessedEntSeq: "9000101" })));
      const before = fs.readFileSync(path);
      const source = capture(xml(9000102, "failed short batch"), "failed.xml");
      const batch = await accepted(source);
      expect(batch.candidates.length).toBeLessThan(1000);
      const failing = {
        upsertBatch: (
          candidates: typeof batch.candidates,
          options?: { dryRun?: boolean; conflictPolicy?: "abort" | "update" },
        ) => adapter.upsertBatch(candidates, { ...options, injectFailureAfterWrites: true }),
      };
      await expect(commitSourceBatch(source, batch, failing, false, {
        path,
        targetIdentity: target,
        progress,
      ...
      expect(rows.map((row) => row.id)).toEqual(["de-jmdict-9000101"]);
      expect(fs.readFileSync(path)).toEqual(before);
      expect(fs.readFileSync(path, "utf8")).not.toContain("9000102");
    } finally {
      await client.close();
    }
  });

  it("commits 717 rows, resumes from that cursor, and retries only after a failed short batch", async () => {
    const client = new PGlite();
    const database = drizzle(client);
    state.database = database;
    const count = 717;
    ...
    try {
      ...
      const source = capture(
        `<JMdict>${Array.from({ length: count }, (_, index) => entry(start + index, `gloss ${index}`)).join("")}</JMdict>`,
        "short-717.xml",
      );
      const batch = await accepted(source);
      expect(batch.candidates.length).toBe(count);
      expect(count).toBeLessThan(1000);
      const path = join(directory, "resume-717.json");
      const committed = await commitSourceBatch(source, batch, adapter, false, {
        path,
        targetIdentity: target,
        progress: { ...progress, recordsProcessed: count },
      });
      expect(committed.inserted).toBe(count);
      expect(await database.select({ id: dictionaryEntries.id }).from(dictionaryEntries)).toHaveLength(count);
      const saved = loadResumeCheckpoint(path, source, target);
      expect(saved.checkpointVersion).toBe(2);
      expect(saved.origin).toBe("ingestion");
      expect(saved.lastProcessedEntSeq).toBe(String(start + count - 1));
      expect(saved.sourceHash).toBe(source.xmlSha256);
      expect(saved.targetIdentity).toEqual(target);

      const repeated = await commitSourceBatch(source, batch, adapter, false, {
        path,
        targetIdentity: target,
        progress: { ...progress, recordsProcessed: count, recordsInserted: count },
      });
      expect(repeated).toEqual({ batchSize: count, inserted: 0, updated: 0, skipped: count });
      expect(await database.select({ id: dictionaryEntries.id }).from(dictionaryEntries)).toHaveLength(count);
      expect(loadResumeCheckpoint(path, source, target).lastProcessedEntSeq).toBe(saved.lastProcessedEntSeq);

      const changed = capture(
        `<JMdict><entry><ent_seq>${start}</ent_seq><k_ele><keb>改</keb></k_ele><r_ele><reb>かい</reb></r_ele><sense><pos>&n;</pos><gloss>changed</gloss></sense></entry></JMdict>`,
        "conflict.xml",
      );
      const conflict = await accepted(changed);
      const beforeConflict = fs.readFileSync(path);
      await expect(commitSourceBatch(changed, conflict, adapter, false, {
        path: join(directory, "conflict-checkpoint.json"),
        targetIdentity: target,
        progress,
      })).rejects.toThrow(/CONFLICT\] ABORT/);
      expect((await database.select().from(dictionaryEntries)).find((row) => row.id === `de-jmdict-${start}`)?.headword).toBe("水");
      expect(fs.readFileSync(path)).toEqual(beforeConflict);
      const updated = await commitSourceBatch(changed, conflict, adapter, false, undefined, "update");
      expect(updated.updated).toBe(1);
      expect((await database.select().from(dictionaryEntries)).find((row) => row.id === `de-jmdict-${start}`)?.headword).toBe("改");
      expect(fs.readFileSync(path)).toEqual(beforeConflict);

      const nextStart = start + count;
      const nextSource = capture(
        `<JMdict>${Array.from({ length: 5 }, (_, index) => entry(nextStart + index, `next ${index}`)).join("")}</JMdict>`,
        "next-short.xml",
      );
      const nextBatch = await accepted(nextSource);
      const beforeFailure = fs.readFileSync(path);
      const failing = {
        upsertBatch: (
          candidates: typeof nextBatch.candidates,
          options?: { dryRun?: boolean; conflictPolicy?: "abort" | "update" },
        ) => adapter.upsertBatch(candidates, { ...options, injectFailureAfterWrites: true }),
      };
      await expect(commitSourceBatch(nextSource, nextBatch, failing, false, {
        path,
        targetIdentity: target,
        progress: { ...progress, recordsProcessed: count + 5, recordsInserted: count },
      })).rejects.toThrow(/injected persistence failure/);
      const afterFailure = await database.select({ id: dictionaryEntries.id }).from(dictionaryEntries);
      expect(afterFailure).toHaveLength(count);
      expect(afterFailure.some((row) => row.id === `de-jmdict-${nextStart}`)).toBe(false);
      expect(fs.readFileSync(path)).toEqual(beforeFailure);
      const retryCursor = loadResumeCheckpoint(path, source, target);
      expect(retryCursor.lastProcessedEntSeq).toBe(String(start + count - 1));
      expect(() => loadResumeCheckpoint(join(directory, "missing-resume.json"), source, target)).toThrow(/missing/);

      const retried = await commitSourceBatch(nextSource, nextBatch, adapter, false, {
        path,
        targetIdentity: target,
        progress: { ...progress, recordsProcessed: count + 5, recordsInserted: count },
      });
      expect(retried.inserted).toBe(5);
      const finalRows = await database.select({ id: dictionaryEntries.id }).from(dictionaryEntries);
      expect(finalRows).toHaveLength(count + 5);
      const resumed = loadResumeCheckpoint(path, nextSource, target);
      expect(resumed.lastProcessedEntSeq).toBe(String(nextStart + 4));
      expect(Number(resumed.lastProcessedEntSeq)).toBeGreaterThan(Number(retryCursor.lastProcessedEntSeq));
      expect(finalRows.every((row) => {
        const seq = Number(row.id.replace("de-jmdict-", ""));
        return seq <= Number(retryCursor.lastProcessedEntSeq) || seq >= nextStart;
      })).toBe(true);
    } finally {
      await client.close();
    }
  ...
  it("keeps the final short batch on the same post-commit checkpoint binding", () => {
    const source = fs.readFileSync(join(process.cwd(), "scripts/ingest-full-jmdict.ts"), "utf8");
    const start = source.indexOf("const commitIngestionBatch");
    const end = source.indexOf("const durationMs");
    expect(start).toBeGreaterThan(0);
    expect(end).toBeGreaterThan(start);
    const block = source.slice(start, end);
    expect(block.match(/commitSourceBatch\(/g)).toHaveLength(1);
    expect(block).toContain("options.dryRun ? undefined : {");
    expect(block).toContain("path: checkpointPath");
    expect(block).toContain("targetIdentity");
    const partial = block.slice(block.indexOf("Flush remaining partial batch"));
    expect(partial).toContain("await commitIngestionBatch(currentBatch)");
