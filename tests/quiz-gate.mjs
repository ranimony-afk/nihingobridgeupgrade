  ...
  const structure = await client.query(`SELECT
    ...
    (SELECT count(*) FROM information_schema.tables
      WHERE table_schema='public'
        AND table_name LIKE '%attempt%'
        AND table_name NOT IN ('user_exercise_attempts')) attempt_tables`);
  const s = structure.rows[0];
  check("db: run tables exist", Number(s.run_tables) === 2);
  // `user_exercise_attempts` is the phase 09.4 lesson-exercise audit log, not a
  // competing quiz attempt model, so it is excluded from the competition check.
...
async function httpChecks() {
  ...
  // A position inside the zod range (<= 500) but absent from this run resolves
  // to "question not found in this run" → 404. (Values > 500 fail validation
  // with 400 first, which is a separate input-contract check.)
  const bogus = await post(`/api/quiz/runs/${publicId}/answers`, { position: 250, optionId: 1 });
