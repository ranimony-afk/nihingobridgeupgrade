...
| `tanaka` | Tanaka corpus (`examples.utf.gz`) | http://www.edrdg.org/examples.html | EDRDG Licence | example sentences for grammar points (Japanese + English pairs, upstream id kept in `grammar_examples.external_id`) |
| `grammar-seed` | NihongoBridge curated grammar points | `etl/data/grammar-points.json` | CC BY-SA 4.0 (NihongoBridge) | grammar point titles, summaries, explanations, formation patterns, notes, tags and relations |
| `grammar-structures` | NihongoBridge grammar structure slots | `etl/data/grammar-structures.json` | CC BY-SA 4.0 (NihongoBridge) | ordered slot breakdown of each point (`[V-て] + しまう`) |
| `grammar-mistakes` | NihongoBridge curated common mistakes | `etl/data/grammar-mistakes.json` | CC BY-SA 4.0 (NihongoBridge) | learner error sentences, corrections, explanations, severity |
| `learning-seed` | NihongoBridge learning catalogue | `etl/data/learning-catalog.json` | CC BY-SA 4.0 (NihongoBridge) | course and lesson copy, objectives, duration and publication state |
| `course-architecture` | NihongoBridge course architecture | `etl/data/course-architecture.json` | CC BY-SA 4.0 (NihongoBridge) | modules, prerequisites, tags and explicit lesson knowledge links |
| `lesson-architecture` | NihongoBridge lesson architecture | `etl/data/lesson-architecture.json` | CC BY-SA 4.0 (NihongoBridge) | lesson section structure and authored prose (intro, key points, tips, checkpoints, summaries) |
...
Three attributes are **computed by the ETL**, they are not part of the upstream files:
...
3. `grammar_points.example_count` — number of harvested `grammar_examples` rows.

Grammar **structure slots** (`grammar_structures`) and **common mistakes**
(`grammar_mistakes`) are hand-authored teaching content (CC BY-SA 4.0); they are
never taken from a corpus or from another product.

## Attribution string

```
Kanji, radical and decomposition data from KANJIDIC2 / KRADFILE / RADKFILE;
vocabulary from JMdict; example sentences from the Tanaka corpus — all © EDRDG,
used under the EDRDG licence. Grammar explanations © NihongoBridge, CC BY-SA 4.0.
```
