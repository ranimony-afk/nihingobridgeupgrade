...
> **Operational hazard — index ordering.** `drizzle-kit push` reconciles the
> schema and will **drop** the GIN / `pg_trgm` / expression indexes created by
> `scripts/search-indexes.sql`, because those index types are not representable
> in Drizzle's schema DSL. Always re-apply the SQL file **after** any push:
>
> ```bash
...
> ```
>
> `scripts/provision.sh` and `etl/run-search-index.mjs` already do this in the
> correct order; `node tests/search-gate.mjs` is the canary that detects a
> dropped index.
