...
### `GET /api/grammar/[slug]` — detail payload (07.4)

Beyond the 07.2 fields the detail payload also contains:

| Field | Content |
| --- | --- |
| `structures[]` | `{ id, position, label, content, required, note }` — ordered slot diagram |
| `mistakes[]` | `{ id, position, incorrect, correction, explanation, severity }` |
| `neighbours[]` | `{ slug, title, direction: "previous" \| "next" }` within the same JLPT level |

`?include=` accepts `structures` and `mistakes` in addition to the 07.2 keys.

### `GET /api/grammar/[slug]/structures`

Ordered structural slots of one point.

### `GET /api/grammar/[slug]/mistakes`

Curated learner errors; `?severity=` filters on `common | subtle | critical`.
