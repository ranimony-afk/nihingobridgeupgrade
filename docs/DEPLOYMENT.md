...
node etl/run-grammar-pipeline.mjs               # grammar + corpus evidence
node etl/run-search-index.mjs                   # PostgreSQL search projection + indexes
# NOTE: `drizzle-kit push` drops GIN/pg_trgm indexes. Always re-run
# `psql "$DATABASE_URL" -f scripts/search-indexes.sql` after a schema push.
node tests/knowledge-gate.mjs                   # DB-level gate (no server needed)
node tests/search-gate.mjs                      # search projection/index DB gate
```
...
node tests/grammar-explorer-gate.mjs $BASE
node tests/grammar-detail-gate.mjs $BASE

# PostgreSQL search (phase 08.1)
curl -s "$BASE/api/search?q=water&mode=auto" | head -c 400
curl -s "$BASE/api/search?q=conditonal&mode=fuzzy&types=grammar" | head -c 400
curl -s "$BASE/api/search/stats" | head -c 300
node tests/search-gate.mjs $BASE

# grammar explorer pages (phase 07.3)
for p in /grammar /grammar/explorer /grammar/map /grammar/te-shimau; do
